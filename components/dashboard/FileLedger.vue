...
export default Vue.extend({
  ...
  methods: {
    ...
    async ledgerFiles(page, search){
      ...
      let acct = this.$auth.user.role != UserTypeEnum.PAID ? `/ledger?userId=${this.$auth.user.id}&$sort[updatedAt]=-1&fileName[$like]=${search}%&$skip=${page}` : `/ledger?mainAccountId=${this.$auth.user.id}&$sort[updatedAt]=-1`
