<template>
    ...
    >
      ...
      <span v-if="!isCreator">Do you want these file to be saved as {{`${(file.fileAction+'ed').replace('ee','e')}`}}?</span>
      <span v-else class="w-full text-center block py-0 px-2 pb-8 text-[16px]">Publish file as <span class="capitalize">{{isCreator ? file.fileAction : `${(file.fileAction+'ed').replace('ee','e')}`}}</span>?</span>
...
import AddToPageDrawOrType from '~/components/modals/AddToPageDrawOrType.vue'
  ...
  export default mixins(SaveSignatureInitialsMixin).extend({
    name: 'SavePdfModal',
    components: { SpinnerDottedIcon, CheckedFillIcon, DrawOrTypeModal, AddToPageDrawOrType },
    model: {
      prop: 'visible',
    ...
    computed: {
      ...
      confirmAnnotation(){
        ...
        return {
          ...
          text: (`P${date.getFullYear()}${this.convertToDoubleString(date.getUTCDate())}${this.convertToDoubleString(date.getUTCDay())}${this.convertToDoubleString(date.getUTCHours())}${this.convertToDoubleString(date.getUTCMinutes())}${this.$auth.user.firstName.charAt(0)}${this.$auth.user.lastName.charAt(0)}`).toUpperCase(),
    ...
    methods: {
      ...
      this.$BUS.$emit('signature-update',
        image
      )
    ...
    async convertImageToBase64(src){
      ...
      addToLedger(){
        this.$axios.post(`/ledger?userId=${this.$auth?.user?.id}`, {...this.ledgerInfo})
        .then(()=>{
    ...
    appendConfirmSign(){
        console.log(ExtractFormPdf(this.file.downloadLink, this.confirmAnnotation))
    },
    confirmAllSignAndInitials(){
      if(this.$auth.user.id == this.file.userId)
       return true
      else
      return ExtractFormPdf(this.file.downloadLink)[0].data.filter((item)=> 
      item.hasOwnProperty('uploaded') && item.uploaded == null).length < 1

    },
    async confirmRequest(){
        await this.$axios.post('/pdf-generator',{...ExtractFormPdf(this.file.downloadLink, this.confirmAnnotation)[0]})
        .then((response)=>{
    ...
    async otherRequest(){
        await this.$axios.post('/pdf-generator',{...ExtractFormPdf(this.file.downloadLink)[0]})
        .then((response)=>{
          this.addToLedger()
    ...
    successAction(){
     ...
     onSubmit(){
      ...
      if (!this.isCreator && !this.$auth.user.signatureURL && this.file.fileAction == FileAction.CONFIRM){
