<template>
  <section>
    ...
    <div
      v-else
      v-if="!isLoading"
      class="tools-container-wrapper flex flex-wrap items-center justify-between bg-[#E8EAEC] py-2 w-full gap-x-1 gap-y-2 px-6 text-[#757575] text-base sm:text-2xl"
...
export default {
  ...
  methods: {
    allowAnnotationsSign_Initial(type){
       return (!this.$auth.loggedIn || (this.isCreator && this.isComplete &&
     (type != this.TOOL_TYPE.appendSignature || type != this.TOOL_TYPE.appendInitial)
     )
       )
    },
    undoFunction(){
    ...
    onSignClick() {
      ...
      this.setSelectedType(this.TOOL_TYPE.signature)
        ...
        this.setSelectedType(TOOL_TYPE.appendSignature)
      }
    },
    onInitialsClick() {
      if(!this.$auth.loggedIn) {
      ...
      this.setSelectedType(this.TOOL_TYPE.initial)
        ...
        this.setSelectedType(TOOL_TYPE.appendInitial)
      }
    },
  },
  watch:{
