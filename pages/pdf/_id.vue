...
export default mixins(PdfAuth).extend({
  ...
  methods: {
  scrollToSignInitial(){
    if(this.file.fileAction != FileAction.SIGNED ) return 
     setTimeout(() => {
        let annotationButton = document.querySelectorAll('.annot-button')
        Array.from(annotationButton).forEach((element)=>{
            element.classList.remove('pulse')
        })
        this.filteredAnnotationButton = Array.from(annotationButton).filter((item, index)=>
          !item.hasAttribute('elemFill')
        ) 
        this.filteredAnnotationButton[0].classList.add('pulse')
        this.filteredAnnotationButton[0].scrollIntoView({ block: "center"})
        // let scroll_dist_elem = this.filteredAnnotationButton[0].getBoundingClientRect().top - document.querySelector('.pdf-editor-view').getBoundingClientRect().top
          // $('.pdf-editor-view').animate({ scrollTop: scroll_dist_elem>0 ? scroll_dist_elem : -scroll_dist_elem }, 200);
     }, 100);
    // alert(annotationButton.length)
  },
    isDeletedFunc(){
    ...
    publishFileFunction(){
      if(this.filteredAnnotationButton.length > 0 && this.isSign){
        this.$notify.error({
          title:'Sign',
          message: 'Kindly fill all sign placeholders'
        })
    ...
    exitFileManager(val){
      let is_equal = ( this.file.annotaions == JSON.stringify(this.tools) || 
    ...
    _scrollToConfirm(){
       ...
       let pdfPage =  document.querySelectorAll('.pdf-single-page-outer')
        ...
        setTimeout(() => {this.isBottom = true}, 200 * pdfPage.length);

        $('.pdf-editor-view').animate({ scrollTop: $("#bottom").offset().top }, 200);  
       }   
      },
    _scrollToSign(){
      // if(this.file.fileAction == FileAction.SIGNED && !this.isCreator){
      //   let button = document.querySelectorAll(".annot-button")
      //   button[1].scrollIntoView({behavior: 'smooth', block: "start", inline: "nearest"})
      //   }
    },
     onloadPdfquery(val) {
    ...
    nextPageHandler() {
      if (this.currentPage <= this.pdf.numPages - 1) {
        document.querySelectorAll('.pdf-page')[this.currentPage].scrollIntoView(true)
      }
    },
    _setPdfToolBarCompanyName(){
      this.$axios.get(`/users/?id=${this.file.userId}`)
      .then((response)=>{
          console.log("vvvvvvvvvvvvvvvvvvvvvvvv",response)
          const company = response.data.data[0]
          this.$store.commit('SET_PDF_PAGE_NAME', { name: company.companyName || (company.firstName + ' ' + company.lastName), 
    ...
    _setToolsFromFileAnnotations(){
      this.tools = JSON.parse(this.file.annotaions || `[]`)
        this.tools = this.tools.map((_el, index)=>{
          ...
          this.initialFileAnnotation = JSON.parse(this.file.annotaions || `[]`).map((_el, index)=>{
              return {..._el, id : index + 1}
          })
    },
    setScrollPage() {
      let current =
        Number(document.querySelector('.pdf-editor-view').scrollTop - 
    ...
    sideSetScrollPage(val) {
      this.currentPage = val
       document.querySelectorAll('.pdf-page')[this.currentPage - 1].scrollIntoView(true)
    },
    ...
    placeTool(e, pageNumber, initialPoint) {
      ...
      let { x, y } = (!initialPoint ? this.pointerPos(e, parent || this.$refs.PagesOuter) : initialPoint)
